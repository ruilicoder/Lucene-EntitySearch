...
import entitySearch.index.analyzer.KeywordEntityAnalyzer;
import entitySearch.preprocess.TextReader;

public class KeywordDocumentEntityIndex {
	
	IndexReader reader;
	public KeywordDocumentEntityIndex(String indexDir) {
		try {
	...
	public static void buildIndex(String fromFile, String fromFile2, String toDir) {
		try {
			BufferedReader reader2 = new BufferedReader(new FileReader(fromFile2));
			String temp = "";
			
			IndexWriter writer= new IndexWriter(toDir + index, new KeywordEntityAnalyzer());
			
			writer.setRAMBufferSizeMB(500);
			writer.setMaxBufferedDocs(500000);
			writer.setMergeFactor(20000);
			
			TextReader reader = new TextReader(fromFile);
			int docNum =  0;
	...
	public static void insert(String fromFile, String fromFile2, String toDir) {
		try {
			BufferedReader reader1 = new BufferedReader(new FileReader(fromFile));
			BufferedReader reader2 = new BufferedReader(new FileReader(fromFile2));
			String temp = "";
	...
	public static Document buildDocument(String content, String entity) {
		...
		doc.add(new Field(Fields.TEXT,  content + " entityrli " + entity, Field.Store.NO, Field.Index.TOKENIZED, Field.TermVector.NO));
		return doc;
	}
}
